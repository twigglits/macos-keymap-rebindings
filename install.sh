...
echo
info "One bonus needs an extra permission: Alt+Tab un-minimizes the focused app's windows."
echo "    If minimized windows don't reappear when you Alt+Tab to an app, grant Accessibility"
echo "    to Karabiner's helper under System Settings → Privacy & Security → Accessibility:"
echo "      /Library/Application Support/org.pqrs/Karabiner-Elements/bin/karabiner_console_user_server"
echo "    then restart the helper so it picks up the grant:"
echo "      launchctl kickstart -k gui/\$(id -u)/org.pqrs.service.agent.karabiner_console_user_server"
